...
$smtp_from_name = 'PM Toilet Yojana Form';      // From name
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $fileFields = [
        'aadhar-husband' => 'Aadhar Card (Husband)',
        'aadhar-wife' => 'Aadhar Card (Wife)',
        'voter-card' => 'Voter Card',
        'ration-card' => 'Ration Card',
        'ghara-pata' => 'Ghara Pata',
        'bank-passbook' => 'Bank Passbook'
    ];
    
    $requiredFileFields = ['aadhar-husband', 'ration-card', 'ghara-pata', 'bank-passbook'];
    ...
    if (empty($errors)) {
        ...
        // Email subject
        $subject = "PM Toilet Yojana Application: $letterNo";
        ...
        $body .= "<html><body>";
        $body .= "<h2>PM Toilet Yojana Application Details</h2>";
        ...
        if ($result) {
            ...
            header("Location: success.php?ref=$letterNo&form=toilet");
            exit();
        } else {
...
if (!$response['success'] && !empty($response['message'])) {
    header("Location: pm-toilet.html?error=" . urlencode($response['message']));
    exit();
}
?>